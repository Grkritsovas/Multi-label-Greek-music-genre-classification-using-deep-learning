...
def get_genre_from_label(target_vector, genre_index_mapping=None):
    """Extract genre from the target vector based on a predefined mapping."""
    if genre_index_mapping is None:
        # Define the mapping of index to genre
        genre_index_mapping = {
            0: "LAIKO",
            1: "REMPETIKO",
            2: "ENTEXNO",
    ...
    # Find the index of 1 in the target vector
    genre_indices = [i for i, val in enumerate(target_vector) if val == 1]
    
    # Return the corresponding genres
    return [genre_index_mapping[idx] for idx in genre_indices if idx in genre_index_mapping]

def process_songs_to_balanced_tfrecord(
    songs_json_path,
    audio_folders,
    ...
    """Process songs into TFRecord files with balanced genre representation."""
    ...
    # Load mapping of YouTube links to local files
    ...
    # Load global stats for audio normalization
    ...
    genre_to_songs = {}
    for song_name, target_vector in songs_data.items():
        genres = get_genre_from_label(target_vector, genre_index_mapping)
        
        # Only consider songs with a single genre for balancing
        if len(genres) == 1:
            genre = genres[0]
            if genre not in genre_to_songs:
                genre_to_songs[genre] = []
            genre_to_songs[genre].append(song_name)
    ...
    # Select songs for balanced dataset
    selected_songs = []
    selected_songs_by_genre = {}
    
    for genre, target in genre_targets.items():
        if genre not in genre_to_songs:
            print(f"Warning: Genre {genre} not found in dataset")
            continue
            
        available_songs = genre_to_songs[genre]
        
        # Select all songs or the target number, whichever is smaller
        if target == "all" or len(available_songs) <= target:
            selected_for_genre = available_songs
            print(f"Using all {len(selected_for_genre)} songs for {genre}")
        else:
            # Randomly sample the target number of songs
            selected_for_genre = random.sample(available_songs, target)
            print(f"Randomly sampled {len(selected_for_genre)} out of {len(available_songs)} songs for {genre}")
        
        selected_songs.extend(selected_for_genre)
        selected_songs_by_genre[genre] = selected_for_genre
    
    # Save the selected songs list for reference
    selected_songs_path = os.path.join(output_dir, f"{split_name}_selected_songs.json")
    with open(selected_songs_path, 'w') as f:
        json.dump(selected_songs_by_genre, f, indent=2)
    
    print(f"Selected {len(selected_songs)} total songs for balanced dataset")
    
    # Randomize the order of songs to process
    random.shuffle(selected_songs)
    
    # Start processing from the current progress index
    progress = load_progress(progress_file)
    start_index = progress.get("current_index", 0)
    
    if start_index >= len(selected_songs):
        print(f"All {len(selected_songs)} songs have already been processed.")
        return
    ...
    for batch_start in range(start_index, len(selected_songs), batch_size):
        batch_end = min(batch_start + batch_size, len(selected_songs))
        batch_songs = selected_songs[batch_start:batch_end]
        ...
        print(f"Progress updated: {batch_end}/{len(selected_songs)} songs processed.")
    ...
    for song in successful_songs:
        # Skip augmented songs in the count
        if "_aug" in song:
            continue
        
        # Get genre for this song
        if song in songs_data:
            genres = get_genre_from_label(songs_data[song], genre_index_mapping)
            if len(genres) == 1:
                genre = genres[0]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Process songs into balanced TFRecord files containing mel spectrograms."
    )
    parser.add_argument("--songs_json", type=str, required=True, 
                        help="Path to the JSON file with song names and labels")
    parser.add_argument("--output_dir", type=str, default="balanced_tfrecord_dataset", 
    ...
    if args.reset_progress and args.progress_file:
        if os.path.exists(args.progress_file):
            os.remove(args.progress_file)
            print(f"Reset progress file: {args.progress_file}")
        else:
            print(f"No progress file found at {args.progress_file}")
    
    # Define genre target counts as specified
    genre_targets = {
        "LAIKO": 120,
        "REMPETIKO": 120,
        "ENTEXNO": 120,
...
""")
    ...
    process_songs_to_balanced_tfrecord(
