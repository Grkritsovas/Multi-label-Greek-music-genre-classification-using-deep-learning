...
def create_example(mel_spec, target_vector, song_name, segment_idx, n_segments):
    """Create a tf.train.Example for a single segment."""
    # Serialize the numpy arrays
    mel_spec_bytes = tf.io.serialize_tensor(mel_spec).numpy()
...
def process_songs_to_tfrecord(
    ...
    # Get the song names as a list
    song_names = list(songs_data.keys())
    
    # Check if we've already processed all songs
    ...
    for batch_start in range(start_index, len(song_names), batch_size):
        ...
        for song_name in tqdm(batch_songs):
            ...
            try:
                ...
                # Apply augmentation if required (for training data)
