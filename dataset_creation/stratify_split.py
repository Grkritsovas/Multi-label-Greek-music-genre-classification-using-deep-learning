...
# Only keep rows that correspond to songs existing in directories
...
def stratified_split_multilabel(df, song_col, label_cols, test_size=0.15, random_seed=42):
    ...
    labels = df[label_cols].applymap(clean_label).values  # label matrix shape = (num_samples, num_labels)
    
    # X is the song names in a 2D array for iterative_train_test_split
...
if __name__ == "__main__":
    ...
    dataset_path = "Greek_Music_Dataset.xlsx"
    ...
    with open("../json/train_songs.json", "w", encoding="utf-8") as f:
        json.dump(train_data_dict, f, indent=2, ensure_ascii=False)
    print("[INFO] Wrote train_songs.json")

    with open("../json/test_songs.json", "w", encoding="utf-8") as f:
